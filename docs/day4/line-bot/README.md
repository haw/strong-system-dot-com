...
2. 以下を追加:
   ...
   - 値: [上記で取得した](#チャネルシークレット-と-チャネルアクセストークン)チャネルアクセストークン
   - キー: `LINE_CHANNEL_SECRET`
   - 値: [上記で取得した](#チャネルシークレット-と-チャネルアクセストークン)チャネルシークレット
3. 「保存」
...
### Step 4: Function URLを有効化

1. 「設定」タブ →「関数URL」→「関数URLを作成」
...
2. LINE Developers の「Webhook URL」に貼り付け
