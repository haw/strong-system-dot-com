...
**Day 1-3との違い**:
- ✅ サーバー管理不要（EC2なし。セキュリティパッチの適用不要、OSのアップグレード不要）
- ✅ 自動スケーリング
- ✅ 従量課金（使った分だけ）
- ✅ 高可用性（マルチAZ自動）
...
- **追加**: 「➕ 従業員を追加」ボタン（1名追加してください）
- **編集**: 各行の「編集」ボタン（任意の1名の名前を変更してください）
- **削除**: 各行の「削除」ボタン（任意の1名を削除してください）

`CRUD = Create, Read, Update, Delete` が問題なく行えることを確認してください。
...
2. 「📤 ファイルを選択」でファイルを選択
3. 「⬆️ アップロード」をクリック
    - 適当なファイルがない方は、 [https://haw.github.io/strong-system-dot-com/day4/images/architecture.png](https://haw.github.io/strong-system-dot-com/day4/images/architecture.png) をダウンロードしてお使いください
4. アップロードしたファイルは：
    - **ダウンロード**: 「ダウンロード」ボタン
    - **削除**: 「削除」ボタン
